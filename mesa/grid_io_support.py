...
def make_sobol_filenames(adjpars, summary_file='./grid.summary', inlist_prefix=None, grid_prefix=None, grid_type=None, worker_prefix=None):
    parts              = []
    combinations       = []
    directory_segments = []
    keys               = adjpars.keys()
    ...
    # for ii,combination in enumerate(itertools.product(*parts):
    for ii in range(len(parts[0])):
        stack         = np.hstack([part[ii] for part in parts])
        print 'STACK: ',stack
...
def write_mesa_inlist_zams(  inlist, history_file, logsdir,
        ...
        replacements = {
		        ...
		        'ZAMS_PROFILE_FILENAME'         : '{}'.format("'"+zams_profile_file+"'"),
		        'ZAMS_PULSE_FILENAME'           : '{}'.format("'"+zams_pulse_file+"'"),
                'STAR_HISTORY_NAME'             : '{}'.format("'"+history_file+"'"),
                'LOGS_DIR'                      : '{}'.format("'"+logsdir+"'"),
                'INITIAL_Y'                     : '{:6.5f}d0'.format(initial_Y),
                'INITIAL_Z'                     : '{:6.5f}d0'.format(initial_Z),
...
def write_mesa_inlist_ms_transition(  inlist, history_file, logsdir,
                                 initial_Y, initial_Z, initial_mass,
                                 mlt_alpha, log_minDmix, log_Dext,
                                 overshoot_f, step_overshoot_f,
                                 overshoot_f0=0.002,
                                 zams_model_file='zams.model',
                                 tams_model_file = 'tams.model',
                                 tams_profile_file = 'tams.profile',
        ...
        replacements = {
                ...
                'INITIAL_MASS'                  : '{:8.6f}d0'.format(initial_mass),
                'OVERSHOOT_F_ABOVE_BURN_H_CORE' : '{:5.4f}d0'.format(overshoot_f+overshoot_f0),
                'STEP_OVERSHOOT_F_ABOVE_BURN_H_CORE' : '{:5.4f}d0'.format(step_overshoot_f+overshoot_f0),
                'MIXING_LENGTH_ALPHA'           : '{:5.4f}d0'.format(mlt_alpha),
                'MIN_D_MIX'                     : '{:8.4f}d0'.format(10**log_minDmix),
                'D_EXT'                         : '{:5.4f}d0'.format(10**log_Dext),
                        }
...
def write_mesa_inlist_post_ms(  inlist, history_file, logsdir,
                                ...
                                overshoot_f0=0.002,
                                tams_model_file='tams.model',
                                rgbtip_model_file = 'rgbtip.model',
                                rgbtip_profile_file = 'rgbtip.profile',
                                base_inlist=os.path.expandvars('$VSC_DATA/python/mesa/inlist_MAMSIE_BASE_VSCGRID')):
        ...
        replacements = {
		        ...
		        'RGBTIP_PROFILE_FILENAME'       : '{}'.format("'"+rgbtip_profile_file+"'"),
                'STAR_HISTORY_NAME'             : '{}'.format("'"+history_file+"'"),
                'LOGS_DIR'                      : '{}'.format("'"+logsdir+"'"),
                'INITIAL_Y'                     : '{:6.5f}d0'.format(initial_Y),
                'INITIAL_Z'                     : '{:6.5f}d0'.format(initial_Z),
