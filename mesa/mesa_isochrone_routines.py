...
def generate_primary_eeps(track):
    ...
    eep1 = { key:track[key][pms_ :zams_] for key in list(track.keys()) } ## PMS  to ZAMS
    eep2 = { key:track[key][zams_:iams_] for key in list(track.keys()) } ## ZAMS to IAMS
    eep3 = { key:track[key][iams_:tams_] for key in list(track.keys()) } ## IAMS to TAMS
    eep4 = { key:track[key][tams_:rgb_ ] for key in list(track.keys()) } ## TAMS to RGBTip
...
def construct_eep(track,npoints=100,keys=None):
    ...
    if keys is not None:
        ...
        keys = np.array( [ key for key in list(primary_eeps[0].keys()) ] )
...
def interpolate_mass_tracks(tracks, masses, i_masses):
    ...
    for kk in range(npoints):
        for jj,key in enumerate(keys):
            ...
            zipped = list(zip(masses,quantity_at_npoint))
            zipped.sort(key=lambda x:x[0])
            sorted_masses,sorted_quantity = list(zip(*zipped))
    ...
    for cc,i_age in enumerate(i_ages):
        ...
        for n_eep in range(npoints):
            ...
            if ( ( min(eep_ages) < i_age < max(eep_ages) ) & (len(eep_masses) > 3) ) :
                ...
                zipped = list(zip(eep_ages,eep_masses))
                zipped.sort(key=lambda x:x[0])
                sorted_eep_ages,sorted_eep_masses = list(zip(*zipped))
                ...
                for key in keys:
                    ...
                    zipped = list(zip(eep_masses, cval))
                    zipped.sort(key=lambda x:x[0])
                    sorted_eep_masses, sorted_cval = list(zip(*zipped))
